...
- Licensing headers on the main source files say **MIT + Commons Clause (commercial use prohibited)**, matching `LICENSE`, even though `package.json` still says `MIT`. Keep the header when adding files that carry one.
